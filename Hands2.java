...
public class Hands2 {
	...
	public static Hands2 createHands(int numCards) {
		Hands2 h = new Hands2();
	...
	private static int infiniteWarPermCards(ArrayList<String> str) {
		...
		for(int s = 0; s < str.size(); s++) {
			if(s >= str.size() * percent) {
				counter++;
				percent = counter * 0.001;
//				System.out.println("====\tPercent Complete: " + ((int)(percent*1000))/10.0 + "%\t   ====");
			}
	 ...
	 * Uses HashMaps to keep track of previous Hands as String values which are used as the key.
	...
	public boolean infiniteWarGame() {
		...
		Hands2 initialHand = cloneHand(this);
		int status = 0;
		...
		while(handOne.size() > 0 && handTwo.size() > 0 && !test.containsKey(handsToString(this))) {
			test.put(handsToString(this), "");
			playWar(status);
		}
		...
		while(unknownValues(initialHand)) {
			...
			while(!test.containsKey(handsToString(this))) {
				test.put(handsToString(this), "");
				playWar(status);
...
//		System.out.println(initialHand.toCardValue());		
	 ...
	 */
	private Hands2 updateLetterValues(Hands2 initial) {
	 ...
	 */
	private boolean duplicateLetters(Hands2 initial) {
	 ...
	 */
	private Hands2 updateDuplicateLetterValues(Hands2 initial) {
		int size = Math.min(initial.handOne.size(), initial.handTwo.size());
	 ...
	 * @return false otherwise.
	 */
	private boolean unknownValues(Hands2 h) {
	...
	/***
	 * Converts the given Hands into a String, "." indicates separation between Hands
	 * @param h, any set of Hands
	 * @return String representing the Card Values of Hands, separated by ".".
	 */
	private String handsToString(Hands2 h) {
		String output = "";
		
		for(int i = 0; i < h.handOne.size(); i++) {output += h.handOne.get(i).getCardValue() + ",";}
		output += ".";
		for(int i = 0; i < h.handTwo.size(); i++) {output += h.handTwo.get(i).getCardValue() + ",";}
	 ...
	 */
	private static Hands2 createRandomHands(int numCards) {
		Hands2 h = new Hands2();
	 ...
	 */
	private static Hands2 splitCards(String str) {
		Hands2 h = new Hands2();
		Cards c = new Cards();
	 ...
	 */
	private Hands2 cloneHand(Hands2 original) {
		Hands2 copy = new Hands2();
		Cards c;
	 ...
	 * @return
	 */
	public static Quene<Hands2> getPermsCards(int numCards){
	 ...
	 */
	private static Quene<Hands2> getPermsCards(ArrayList<String> str) {
		Quene<Hands2> output = new Quene<Hands2>();
